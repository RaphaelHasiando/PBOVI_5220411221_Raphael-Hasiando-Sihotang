...
from app import cursor, connect
...
class SosialMasyarakat(KegiatanSosial):
    ...
    def gotong_royong(self):
        ...
        cursor.execute(f"UPDATE sosial_masyarakat_users SET sosialisasi_poin = {self.__sosialisasi} WHERE NIK = '{self.NIK}'")
        connect.commit()
...
def CRUD():
    ...
    print("3.   Hapus Data")
    print("4.   Tampil Data")
    print("5.   Hapus Data dalam Tabel MySQL")
    print("0.   Keluar")
    # Tambahan
    ...
    if kondisi is True or kondisi is False:
        ...
        if kondisi is True:
            ...
            for i in objects_kampus:
                if i.NIK == hdf[7]:
                    x = True
                    z_object = objects_masyarakat[y]
                    break
                y += 1
            # Jika salah satu masyarakat sudah daftar atau membuat objek kampus
            if x is True:
                kampus(z_object)
                return
            
            # INSERT DATA KE TABLE YANG SUDAH DISEDIAKAN (sosial_users dan sosial_masyarakat_users)
            cursor.execute(f"INSERT INTO sosial_users (nama, alamat, tempat_lahir, tanggal_lahir) VALUES ('{hdf[0]}', '{hdf[1]}', '{hdf[2]}', '{hdf[3]}')")

            connect.commit()
            objects_masyarakat_dan_kampus.append(KegiatanSosial(hdf[0], hdf[1], hdf[2], hdf[3]))

            cursor.execute("SELECT id FROM sosial_users")
            result = cursor.fetchall()
            val = (result[-1][0], hdf[4], hdf[5], hdf[6], 0)
            cursor.execute("INSERT INTO sosial_masyarakat_users (id, NIK, kelurahan, kecamatan, sosialisasi_poin) VALUES (%i, '%s', '%s', '%s', '%i')" % val)
            connect.commit()
        ...
        if kondisi is False:
            ...
            for i in objects_kampus:
                if i.npm == hdf[7]:
                    x = True
                    print(x)
                    z_object = objects_kampus[y]
                    break
                y += 1
            # Jika salah satu mahasiswa sudah daftar atau membuat objek kampus
            if x is True:
                print(f"Maaf, Data yang anda isi sudah terdaftar, apakah anda ingin melanjutkan sebagai: {z_object.nama} dengan npm {z_object.npm}?")
                input_user = input("Ketik YA jika ingin dilanjutkan ==> ")
                if input_user == "YA":
                    kampus(z_object)
                    return
                return
            
            # INSERT DATA KE TABLE YANG DISEDIAKAN (sosial_users dan sosial_kampus_users)
            cursor.execute(f"INSERT INTO sosial_users (nama, alamat, tempat_lahir, tanggal_lahir) VALUES ('{hdf[0]}', '{hdf[1]}', '{hdf[2]}', '{hdf[3]}')")
            ...
            cursor.execute("SELECT id FROM sosial_users")
            result = cursor.fetchall()
            val = (result[-1][0], hdf[4], hdf[5], hdf[6], hdf[7])
            cursor.execute("INSERT INTO sosial_kampus_users (id, nama_kampus, jurusan, fakultas, npm) VALUES (%i, '%s', '%s', '%s', '%s')" % val)
            connect.commit()
        ...
        if kondisi == 1:
            ...
            for i in objects_kampus:
                if i.npm == hdf[7]:
                    x = True
            # Jika salah satu mahasiswa belum daftar atau membuat objek kampus
            if x is False:
                objects_kampus.append(SosialKampus(hdf[0], hdf[1], hdf[2], hdf[3], hdf[4], hdf[5], hdf[6], hdf[7]))    
            

            cursor.execute(f"SELECT id FROM sosial_kampus_users WHERE npm = '{hdf[7]}'")
            ...
            if result is None:
                ...
                for i in hdf[9]:
                    cursor.execute(f"INSERT INTO sosial_kelas_users (id, kelas, nama_matkul) VALUES ({result[-1][0]}, '{hdf[8]}', '{i}')")
                    connect.commit()
                objects_masyarakat_dan_kampus.append(KegiatanSosial(hdf[0], hdf[1], hdf[2], hdf[3]))
            else:
                for i in hdf[9]:
                    cursor.execute(f"INSERT INTO sosial_kelas_users (id, kelas, nama_matkul) VALUES ({result}, '{hdf[8]}', '{i}')")
                    connect.commit()
...
def tampil_table(kondisi = None):
    if kondisi is True or kondisi is False:
        if kondisi is True:
            cursor.execute("SELECT sosial_masyarakat_users.NIK, sosial_masyarakat_users.kelurahan, sosial_masyarakat_users.kecamatan, sosial_masyarakat_users.sosialisasi_poin, sosial_users.nama FROM sosial_masyarakat_users INNER JOIN sosial_users ON sosial_masyarakat_users.id = sosial_users.id")
            result = cursor.fetchall()
            myTable = PrettyTable(["No.", "Nama", "NIK","Kelurahan", "Kecamatan", "Sosialisasi Poin"])
            x = 0
            for i in result:
                x+= 1
                myTable.add_row([x, i[4], i[1], i[0], i[2], i[3]])
            print(myTable)
            myTable.clear()
            return
        if kondisi is False:
            cursor.execute("SELECT sosial_kampus_users.npm, sosial_kampus_users.nama_kampus, sosial_kampus_users.jurusan, sosial_kampus_users.fakultas, sosial_users.nama FROM sosial_kampus_users INNER JOIN sosial_users ON sosial_kampus_users.id = sosial_users.id")
            result = cursor.fetchall()
            myTable = PrettyTable(["No.","Nama", "NPM", "Nama_Kampus", "Jurusan", "Fakultas"])
            x = 0
            for i in result:
                x+= 1
                myTable.add_row([x, i[4], i[0], i[1], i[2], i[3]])
            print(myTable)
            myTable.clear()
            return
    if kondisi == 1:
        cursor.execute("SELECT sosial_kampus_users.npm, sosial_kelas_users.kelas, sosial_kelas_users.nama_matkul FROM sosial_kelas_users INNER JOIN sosial_kampus_users ON sosial_kelas_users.id = sosial_kampus_users.id")
        result = cursor.fetchall()
        myTable = PrettyTable(["No.","NPM","KELAS", "NAMA Matakuliah"])
        x = 0
        for i in result:
            x+= 1
            myTable.add_row([x, i[0], i[1], i[2]])
        print(myTable)
        myTable.clear()
        return

    cursor.execute("SELECT nama, alamat, tempat_lahir, tanggal_lahir FROM sosial_users")
    result = cursor.fetchall()
    myTable = PrettyTable(["No.","Nama","Alamat", "Tempat Lahir", "Tanggal Lahir"])
    x = 0
    for i in result:
        x+= 1
        myTable.add_row([x, i[1], i[0], i[2], i[3]])
    print(myTable)
    myTable.clear()
...
def tampil_data(kondisi):
    if kondisi is True or kondisi is False:
        if kondisi is True:
            myTable = PrettyTable(["No.","Nama","NIK"])
            x = 0
            for i in objects_masyarakat:
                x += 1
                myTable.add_row([x, i.nama, i.NIK])
            print(myTable)
            myTable.clear
            print("\n")
            
        if kondisi is False:
            myTable = PrettyTable(["No.","Nama","NPM"])
            x = 0
            for i in objects_kampus:
                x += 1
                myTable.add_row([x, i.nama, i.npm])
            print(myTable)
            myTable.clear
            print("\n")
    else:
...
def hapus_data(kondisi):
    if kondisi is True or kondisi is False:
        if kondisi is True:
            cursor.execute("SELECT * FROM sosial_masyarakat_users")
        if kondisi is False:
            objek = objects_kampus
    else:
        if kondisi == 1:
...
def hapus_tabel_data():
    cursor.execute("SELECT nama, alamat, tempat_lahir, tanggal_lahir, id FROM sosial_users")
    result = cursor.fetchall()
    if result is None:
        print("Data dalam tabel masing kosong")
        return
    myTable = PrettyTable(["ID","Nama","Alamat", "Tempat Lahir", "Tanggal Lahir"])
    kumpulan_id = []
    for i in result:
        kumpulan_id.append(i[4])
        myTable.add_row([i[4], i[1], i[0], i[2], i[3]])
    print(myTable)
    myTable.clear()

    print("Pilih Tabel yang ingin dihapus")
    while True:
        user_input = int(input("==> "))
        if user_input in kumpulan_id:
            cursor.execute(f"DELETE FROM sosial_kelas_users WHERE id = {user_input}")
            connect.commit()
            cursor.execute(f"DELETE FROM sosial_kampus_users WHERE id = {user_input}")
            connect.commit()
            cursor.execute(f"DELETE FROM sosial_masyarakat_users WHERE id = {user_input}")
            connect.commit()
            cursor.execute(f"DELETE FROM sosial_users WHERE id = {user_input}")
            connect.commit()
            break
        else:
            print("Input Invalid")
